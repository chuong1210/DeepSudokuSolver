...
def displayImageSolution(image, solution, original_grid, largest_rect_coord):
    ...
    for i in range(9):
        for j in range(9):
            num = solution[i][j]
            if num != 0:
                # Tính tọa độ chính giữa của mỗi ô
                ...
                # Vẽ số lên frame gốc
                # Sử dụng font chữ và kích thước hợp lý để tránh bị nghiêng
                cv2.putText(image, str(num), (int(point_frame[0] - 10), int(point_frame[1] + 10)), 
