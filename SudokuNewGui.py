...
import GA as gss
from sudokuSolverAstart import solveAStar, solveBFS, solveDFS
...
class SudokuApp(ttk.Window):
    ...
    def draw_grid(self):
        ...
        for i in range(self.grid_size + 1):
            ...
            self.canvas.create_line(i * self.cell_size, 0, i * self.cell_size, h, width=line_width)
            self.canvas.create_line(0, i * self.cell_size, w, i * self.cell_size, width=line_width)
    ...
    def create_control_panel(self):
        ...
        undo_button = ttk.Button(undo_frame, text="Hoàn tác", image=self.undo_icon, compound=LEFT, command=self.undo_move)
        ...
        ttk.Button(self.right_frame, text="Kiểm tra giải pháp", command=self.check_solution).pack(fill=X, expand=YES, pady=10)
    ...
    def new_game(self):
        ...
        if self.grid_size == 9:
            ...
            if level == "Dễ":
                ...
                puzzle = "0" * 81  # Empty grid if level is invalid
        ...
        # Ensure the puzzle is converted to a list of integers
    ...
    def solve(self):
        ...
        if self.solution_grid is not None:
            self.editable_grid = np.copy(self.solution_grid)
            self.update_grid_display()
